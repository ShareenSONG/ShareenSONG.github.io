...
export default function Home() {
  return (
    <main id="main-content" data-title-zh="宋艾欣 — AI 产品经理" data-title-en="Shareen Song — AI Product Manager" data-description-zh="宋艾欣的个人作品集：探索 AI、产品与人类行为如何交汇。" data-description-en="Shareen Song's portfolio: exploring how AI, products and human behavior come together.">
      <section className="identity-hero" id="top" aria-labelledby="hero-title">
        <div className="hero-mesh" aria-hidden="true" />
        <Image className="hero-image" src="/images/shareen-portrait.jpg" alt="宋艾欣在柔和日光下的肖像" data-alt-zh="宋艾欣在柔和日光下的肖像" data-alt-en="Portrait of Shareen Song in soft daylight" fill sizes="(max-width: 760px) 55vw, 31vw" priority />
        ...
        <div className="hero-actions" aria-label="联系与个人资料 / Contact and profile links">
          <a href={`mailto:${contact.email}`} data-cursor="MAIL"><Localized zh="邮箱" en="EMAIL" /> ↗</a>
          <a href={contact.github} target="_blank" rel="noreferrer" data-cursor="OPEN">GITHUB ↗</a>
          <a href={contact.resume} target="_blank" rel="noreferrer" data-cursor="OPEN"><Localized zh="简历" en="RESUME" /> ↗</a>
        </div>
        <a href="#about" className="scroll-cue" aria-label="向下浏览关于我 / Scroll to about" data-cursor="DOWN"><Localized zh="继续了解我" en="SCROLL TO KNOW ME" /><i aria-hidden="true">↓</i></a>
      </section>
      ...
      <section className="about-section light-section" id="about" aria-labelledby="about-title">
        ...
        <div className="about-grid">
          ...
          <div className="about-copy">
            ...
            <ul aria-label="工作原则 / Working principles">
        ...
        <div className="project-stack">
          {projects.map((project) => (
            <article className="project-feature" key={project.slug}>
              <Link href={`/work/${project.slug}`} className="project-art-link" aria-label={`查看 ${project.title.zh} 项目案例 / View ${project.title.en} case study`} data-cursor="VIEW"><ProjectArtwork project={project} /></Link>
              <div className="project-copy">
                ...
                <div className="project-bottom"><p><span><Localized zh="我的角色" en="ROLE" /></span><Copy value={project.role} /></p><Link href={`/work/${project.slug}`} data-cursor="OPEN"><Localized zh="查看案例" en="VIEW CASE" /> <span aria-hidden="true">↗</span></Link></div>
              </div>
            </article>
          ))}
        </div>
        <Link className="text-link" href="/work" data-cursor="GO"><Localized zh="查看全部项目" en="VIEW ALL WORK" /><span>↗</span></Link>
      </section>
      ...
      <section className="thoughts-preview light-section" id="thoughts" aria-labelledby="thoughts-title">
        ...
        <div className="thought-grid">
          {thoughtTopics.map((thought) => {
            ...
            return thought.href ? <a href={thought.href} target="_blank" rel="noreferrer" className="thought-card" data-cursor="READ" key={thought.title.en}>{content}</a> : <article className="thought-card thought-card--draft" key={thought.title.en}>{content}</article>;
          })}
        </div>
        <Link className="text-link" href="/thoughts" data-cursor="GO"><Localized zh="打开思考索引" en="OPEN THOUGHTS INDEX" /><span>↗</span></Link>
      </section>
      ...
      <section className="personal-preview" aria-labelledby="personal-title">
        ...
        <div className="personal-grid">
          <div className="personal-copy"><p><Localized zh="角色之外" en="BEYOND THE ROLE" /></p><h2 id="personal-title"><Localized zh={<>先是一个人，<br /><em>再是一份职位。</em></>} en={<>A person before<br /><em>a job title.</em></>} /></h2><p><Localized zh="旅行、电影、自然、11 年瑜伽、古筝十级，以及那些让产品直觉始终保有人味的微小观察。" en="Travel, cinema, nature, eleven years of yoga, Grade 10 guzheng, and the small observations that keep my product instincts human." /></p><Link href="/life" data-cursor="OPEN"><Localized zh="更多关于我" en="MORE ABOUT ME" /><span>↗</span></Link></div>
          <figure className="personal-image personal-image--wide"><Image src="/images/harbour-sunset.jpg" alt="宋艾欣在维多利亚港日落时分" data-alt-zh="宋艾欣在维多利亚港日落时分" data-alt-en="Shareen by Victoria Harbour at sunset" fill sizes="(max-width: 720px) 100vw, 48vw" /><figcaption><Localized zh="香港 / 日落时分" en="HONG KONG / GOLDEN HOUR" /></figcaption></figure>
          <figure className="personal-image personal-image--portrait"><Image src="/images/un-visit.jpg" alt="宋艾欣参访日内瓦联合国" data-alt-zh="宋艾欣参访日内瓦联合国" data-alt-en="Shareen visiting the United Nations in Geneva" fill sizes="(max-width: 720px) 100vw, 28vw" /><figcaption><Localized zh="日内瓦 / 研学" en="GENEVA / LEARNING EXPEDITION" /></figcaption></figure>
        </div>
      </section>
    </main>
  );
